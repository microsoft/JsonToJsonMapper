...
public static class Program
{
  ...
  private static void Complex2ComplexTransformationSample()
  {
    const string mappingJson = "{\"MappingRuleConfig\":{\"TruthTable\":[{\"DestinationColumn\":\"ContentJSON\",\"ComplexType\":{\"DataType\":\"JArray\",\"Node\":\"$.result[*]\",\"TruthTable\":[{\"SourceColumn\":\"$.leadId\",\"DestinationColumn\":\"leadId\",\"DataType\":\"long\"},{\"SourceColumn\":\"$.attributes[*]\",\"DataType\":\"JArray\",\"TransformValue\":{\"Type\":\"PromoteArrayToProperty\",\"KeyLookupField\":\"$.name\",\"ValueLookupField\":\"$.value\"}}]}}]}}";
    const string inputJson = "{\"result\":[{\"id\":2,\"leadId\":6,\"activityDate\":\"2013-09-26T06:56:35+0000\",\"activityTypeId\":12,\"attributes\":[{\"name\":\"Source Type\",\"value\":\"Web page visit\"},{\"name\":\"Source Info\",\"value\":\"http://search.yahoo.com\"}]}]}";
    var autoMapper = new AutoMapper(mappingJson);
  ...
  private static void RoslynSample()
  {
    //Arrange - Certain input json with complex object
  ...
  private static void TransformToUpperCaseParent()
  {
    string mapping;
    using (var mappingReader = new StreamReader(@"Jsons\ToUpper\Upper_Mapping.json"))
    {
      mapping = JToken.ReadFrom(new JsonTextReader(mappingReader)).ToString();
    }

    string inputJson;
    using (var inputReader = new StreamReader(@"Jsons\ToUpper\Transformation_Input.json"))
    {
      inputJson = JToken.ReadFrom(new JsonTextReader(inputReader)).ToString();
    }
  ...
  private static void TransformToLowerCase()
  {
  ...
  private static void OneToOneMappingTest()
  {
    //Arrange
    string mappingJson;
    using (var mapingReader = new StreamReader(@"Jsons\OneToOneMapping\OneToOneMappingRules.json"))
    {
      mappingJson = JToken.ReadFrom(new JsonTextReader(mapingReader)).ToString();
    }

    string input;
    using (var inputReader = new StreamReader(@"Jsons\OneToOneMapping\Transformation_Input.json"))
    {
      input = JToken.ReadFrom(new JsonTextReader(inputReader)).ToString();
    }
  ...
  private static void RangeMappingTest()
  {
    //Arrange
    string mappingJson;
    using (var mappingReader = new StreamReader(@"Jsons\RangeMapping\RangeMappingRules.json"))
    {
      mappingJson = JToken.ReadFrom(new JsonTextReader(mappingReader)).ToString();
    }

    string input;
    using (var inputReader = new StreamReader(@"Jsons\RangeMapping\Transformation_Input.json"))
    {
      input = JToken.ReadFrom(new JsonTextReader(inputReader)).ToString();
    }
